...
const brandPalette = [
  { name: "Sunbeam", tone: "Primary glow", swatch: "bg-brand-blue" },
  { name: "Amber", tone: "Accent", swatch: "bg-brand-amber" },
  { name: "Sand", tone: "Surface", swatch: "bg-brand-sand" },
  { name: "Slate Night", tone: "Backdrop", swatch: "bg-slate-900" },
];
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-slate-950 text-white">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-6xl flex-col items-center justify-center px-6 py-12 text-center">
        ...
        <motion.div
          className="mt-12 w-full rounded-2xl border border-white/10 bg-slate-900/70 p-5 text-left shadow-lg shadow-black/25 backdrop-blur"
          initial={{ opacity: 0, y: 24 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.9, delay: 0.9 }}
        >
          <div className="mb-4 flex items-center gap-3">
            <div className="relative h-12 w-12 overflow-hidden rounded-xl bg-slate-950/70 p-2 shadow-md shadow-amber-300/25">
              <Image
                src="/Official Logo of HOSL.svg"
                alt="HOSL brand crest"
                fill
                sizes="48px"
                className="object-contain"
              />
            ...
            <div className="text-amber-50">
              <p className="text-xs uppercase tracking-[0.18em] text-amber-200">Brand color board</p>
              <p className="text-base font-semibold">A subtle wash of yellow carried across the experience.</p>
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {brandPalette.map((palette) => (
              <div key={palette.name} className="flex items-center gap-3 rounded-xl bg-white/5 p-3 shadow-inner shadow-black/10">
                <span className={`h-12 w-12 rounded-lg ${palette.swatch} shadow-inner shadow-amber-200/50`} aria-hidden />
                <div className="flex flex-col text-sm text-slate-100">
                  <span className="font-semibold text-amber-50">{palette.name}</span>
                  <span className="text-xs text-amber-100/80">{palette.tone}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      >
        <div className="relative h-20 w-20 rounded-2xl border border-amber-100/20 bg-slate-900/60 p-3 shadow-lg shadow-amber-300/30 backdrop-blur">
          <Image
            src="/Official Logo of HOSL.svg"
