  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-slate-950 text-white">
      ...
      {brandMarks.map((mark) => (
        <motion.div
          key={mark.position}
          className={`pointer-events-none absolute ${mark.position} ${mark.size} z-0`}
      ...
      />
      <motion.div
        className="absolute left-6 top-10 z-10 rounded-full border border-brand-gold/30 bg-white/10 px-4 py-2 text-xs font-semibold text-slate-50 shadow-lg shadow-amber-500/20 backdrop-blur"
        initial={{ opacity: 0, y: -10, scale: 0.9 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
